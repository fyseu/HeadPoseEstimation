...
class SSDLoss:
    ...
    def angle_loss(self, y_true, y_pred):
        angle_absolute_loss = 10 * tf.abs(y_true - y_pred)
        angle_square_loss = 10 * 0.5 * (y_true - y_pred) ** 2
        angle_l1_loss = tf.where(tf.less(angle_absolute_loss, 1.0), angle_square_loss, angle_absolute_loss - 0.5)
        return tf.reduce_sum(angle_l1_loss, axis=-1)
        # # Make sure that `y_pred` doesn't contain any zeros (which would break the log function)
        # # y_pred = tf.maximum(y_pred, 1e-15)
        # # Compute the angle loss
        # # angle_loss = tf.reduce_sum(0.5 * (y_true - y_pred)**2, axis=-1)
        # #角度精度较低，提高angle_loss比重
        # angle_loss = tf.reduce_sum(5 * (y_true - y_pred) ** 2, axis=-1)
        # return angle_loss
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        angle_loss = tf.to_float(self.angle_loss(y_true[:, :, -14:-12], y_pred[:, :, -14:-12]))  # Output shape: (batch_size, n_boxes)
        ...
        # angle_positives = tf.to_float(tf.ones_like(y_true[:, :, 0]))  # Tensor of shape (batch_size, n_boxes)
        ...
        angle_pos_loss = tf.reduce_sum(angle_loss * positives, axis=-1)  # Tensor of shape (batch_size,)
        ...
        total_loss = (angle_pos_loss + class_loss + self.alpha * loc_loss) / tf.maximum(1.0, n_positive) # In case `n_positive == 0`
