...
def _greedy_nms2(predictions, iou_threshold=0.45, coords='minmax'):
    ...
    while boxes_left.shape[0] > 0: # While there are still boxes left to compare...
        ...
        if boxes_left.shape[0] == 0: break # If there are no boxes left after this step, break. Otherwise...
        # similarities = iou(boxes_left[:,2:], maximum_box[2:], coords=coords) # ...compare (IoU) the other left over boxes to the maximum box...
        similarities = iou(boxes_left[:, 5:], maximum_box[5:],coords=coords)  # ...compare (IoU) the other left over boxes to the maximum box...
        boxes_left = boxes_left[similarities <= iou_threshold] # ...so that we can remove the ones that overlap too much with the maximum box
    return np.array(maxima)
...
def decode_y2(y_pred,
    ...
    y_pred_converted = np.copy(y_pred[:, :, -17:-8])
    ...
    y_pred_converted[:, :, 0] = np.argmax(y_pred[:, :, :-15],axis=-1)  # The indices of the highest confidence values in the one-hot class vectors are the class ID
    y_pred_converted[:, :, 1] = np.amax(y_pred[:, :, :-15], axis=-1)  # Store the confidence values themselves, too
    # print(y_pred)
    ...
    if input_coords == 'centroids':
        ...
        y_pred_converted[:,:,[7,8]] = np.exp(y_pred_converted[:,:,[7,8]] * y_pred[:,:,[-2,-1]]) # exp(ln(w(pred)/w(anchor)) / w_variance * w_variance) == w(pred) / w(anchor), exp(ln(h(pred)/h(anchor)) / h_variance * h_variance) == h(pred) / h(anchor)
        y_pred_converted[:,:,[7,8]] *= y_pred[:,:,[-6,-5]] # (w(pred) / w(anchor)) * w(anchor) == w(pred), (h(pred) / h(anchor)) * h(anchor) == h(pred)
        y_pred_converted[:,:,[5,6]] *= y_pred[:,:,[-4,-3]] * y_pred[:,:,[-6,-5]] # (delta_cx(pred) / w(anchor) / cx_variance) * cx_variance * w(anchor) == delta_cx(pred), (delta_cy(pred) / h(anchor) / cy_variance) * cy_variance * h(anchor) == delta_cy(pred)
        y_pred_converted[:,:,[5,6]] += y_pred[:,:,[-8,-7]] # delta_cx(pred) + cx(anchor) == cx(pred), delta_cy(pred) + cy(anchor) == cy(pred)
        ...
        y_pred_converted[:,:,5:] *= y_pred[:,:,-4:] # delta(pred) / size(anchor) / variance * variance == delta(pred) / size(anchor) for all four coordinates, where 'size' refers to w or h, respectively
        y_pred_converted[:,:,[5,6]] *= np.expand_dims(y_pred[:,:,-7] - y_pred[:,:,-8], axis=-1) # delta_xmin(pred) / w(anchor) * w(anchor) == delta_xmin(pred), delta_xmax(pred) / w(anchor) * w(anchor) == delta_xmax(pred)
        y_pred_converted[:,:,[7,8]] *= np.expand_dims(y_pred[:,:,-5] - y_pred[:,:,-6], axis=-1) # delta_ymin(pred) / h(anchor) * h(anchor) == delta_ymin(pred), delta_ymax(pred) / h(anchor) * h(anchor) == delta_ymax(pred)
        y_pred_converted[:,:,5:] += y_pred[:,:,-8:-4] # delta(pred) + anchor == pred for all four coordinates
    ...
    if normalize_coords:
        ...
        y_pred_converted[:, :, 5:7] *= img_width  # Convert xmin, xmax back to absolute coordinates
        y_pred_converted[:, :, 7:] *= img_height  # Convert ymin, ymax back to absolute coordinates

    # print("------------------------------")
    # print(y_pred_converted.shape)
    # print(y_pred_converted)
    # print("------------------------------")
...
class SSDBoxEncoder:
    ...
    via an -intersectionover-union threshold criterion.
    '''
    ...
    def generate_encode_template(self, batch_size, diagnostics=False):
        ...
        ###########################################################################################################
        classes_tensor = np.zeros((batch_size, boxes_tensor.shape[1], self.n_classes + 3))
################################################################################################################
    ...
    def encode_y(self, ground_truth_labels):
        ...
        for i in range(y_encode_template.shape[0]): # For each batch item...
            ...
            for true_box in ground_truth_labels[i]: # For each ground truth box belonging to the current batch item...
                ...
                if (true_box[4] - true_box[3] == 0) or (true_box[6] - true_box[5] == 0): continue # Protect ourselves against bad ground truth data: boxes with width or height equal to zero
                if self.normalize_coords:
                    true_box[3:5] /= self.img_width # Normalize xmin and xmax to be within [0,1]
                    true_box[5:7] /= self.img_height # Normalize ymin and ymax to be within [0,1]
                if self.coords == 'centroids':
                    true_box = convert_coordinates(true_box, start_index=3, conversion='minmax2centroids')
                # similarities = iou(y_encode_template[i,:,-12:-8], true_box[1:], coords=self.coords) # The iou similarities for all anchor boxes
                similarities = iou(y_encode_template[i, :, -12:-8], true_box[3:],coords=self.coords)  # The iou similarities for all anchor boxes
                ...
                if len(assign_indices) > 0: # If we have any matches
                    ...
                    # # print(y_encoded[i, :, :-12])
                    # print(y_encoded[i, assign_indices, :-8])
                    # print("true_box")
                    # # print(y_encoded[i, :, :-12])
                    # print(true_box[2:6])
                    y_encoded[i, assign_indices, :-8] = np.concatenate((class_vector[1], true_box[0:3], true_box[3:7]), axis=0)  # Write the ground truth box coordinates and class to all assigned anchor box positions. Remember that the last four elements of `y_encoded` are just dummy entries.
                    #print(y_encoded[i, assign_indices, -14:-12])
                    ...
                    y_encoded[i, best_match_index, :-8] = np.concatenate((class_vector[1], true_box[0:3], true_box[3:7]), axis=0)  # Write the ground truth box coordinates and class to the best match anchor box position
        ...
        if self.coords == 'centroids':
            ...
            # print(y_encoded[i, :, :-12])
            # print(np.concatenate((class_vector[1], true_box[0:2], true_box[2:6]), axis=0))
            # print(y_encoded[i, assign_indices, :-8])
            # print("y_encode_template++++++++++++++++")
            # # print(y_encoded[i, :, :-12])
            # print(y_encode_template[i, assign_indices, :-8])
