...
class BatchGenerator:
    ...
    def __init__(self,
                 ...
                 box_output_format = ['pitch', 'yaw', 'roll', 'xmin', 'xmax', 'ymin', 'ymax']):
                 ########################################################
        '''
        Arguments:
    ...
    def parse_csv(self,
        ...
        Arguments:
            ...
            input_format (list, optional): A list of six strings representing the order of the six items
                image file name, pitch, yaw, roll, xmin, xmax, ymin, ymax in the input CSV file. The expected strings
                are 'image_name', 'xmin', 'xmax', 'ymin', 'ymax', ', pitch, yaw, roll'. Defaults to `None`.
        ...
        with open(self.labels_path, newline='') as csvfile:
            ...
            for i in csvread: # For every line (i.e for every bounding box) in the CSV file...
                if k == 0: # Skip the header row
                    ...
                    ####################################################################
                    if self.include_classes == 'all' :  # If the class_id is among the classes that are to be included in the dataset...
                    ...
                    ######################################################################
                        ...
                        for item in self.box_output_format: # For each item in the output format...
                         #####################################################################
                            ...
                            obj.append(float(i[self.input_format.index(item)].strip()))  # ...select the respective column in the input format and append it to `obj`
                         #################################################################
                        data.append(obj)
...
#####################################################################################
        yaw = self.box_output_format.index('yaw')
######################################################################################
        while True:
            ...
            for i in range(len(batch_X)):
                ...
                if flip:
                    ...
                    ################################################################################################
                        batch_y[i][:, [yaw]] = -1 * batch_y[i][:, [yaw]]#水平翻转后yaw角翻转
                    ################################################################################################
                if translate:
                    p = np.random.uniform(0,1)
                    if p >= (1-translate[2]):
